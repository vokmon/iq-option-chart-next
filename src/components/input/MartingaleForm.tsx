...
export default function MartingaleForm({
  martingale,
  updateMartingale,
}: MartingaleFormProps) {
  ...
  const handleLevelChange = (value: string | null) => {
    if (value) {
      ...
      const currentLevel = martingale.numberOfMartingales;
      ...
      if (newLevel > currentLevel) {
        ...
        updateMartingale({
          enabled: martingale.enabled,
          numberOfMartingales: newLevel,
          multipliers: newMultipliers,
        });
      } else if (newLevel < currentLevel) {
        // Remove excess levels
        const newMultipliers = martingale.multipliers.slice(0, newLevel);
        updateMartingale({
          enabled: martingale.enabled,
          numberOfMartingales: newLevel,
          multipliers: newMultipliers,
  ...
  const updateMultiplier = (index: number, value: number) => {
    const newMultipliers = [...martingale.multipliers];
    newMultipliers[index] = Math.max(1, value);
    updateMartingale({
      enabled: martingale.enabled,
      numberOfMartingales: martingale.numberOfMartingales,
      multipliers: newMultipliers,
    });
  };
        ...
        {martingale.enabled && (
          <>
            ...
            >
              ...
              <Select
                value={martingale.numberOfMartingales.toString()}
            ...
            >
              <Group gap="sm" mb="md">
                ...
                <div>
                  ...
                  <Text size="sm" c="dimmed">
                    {t("Set individual multipliers for each martingale level", {
              ...
              <Grid gutter="md">
                {martingale.multipliers.map((multiplier, index) => (
              ...
              <div className="mt-4 p-3 bg-green-100 dark:bg-green-900/30 rounded-lg border border-green-200 dark:border-green-700">
                <Text size="sm" c="green.8" fw={500}>
                  {t("Strategy Summary")}: {martingale.numberOfMartingales}{" "}
                  {t("levels")} - {t("Multipliers")}: [
                  {martingale.multipliers.map((m, i) => (
                    <span key={i}>
                      {m}x{i < martingale.multipliers.length - 1 ? ", " : ""}
                    </span>
                  ))}
                  ]
                </Text>
